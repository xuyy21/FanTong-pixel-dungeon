...
public class DM300 extends Mob {
	...
	public void move(int step, boolean travelling) {
		...
		if (!flying && Dungeon.level.map[pos] == Terrain.INACTIVE_TRAP && state == HUNTING) {
			...
			if (CavesBossLevel_DM300.PylonEnergy.volumeAt(pos, CavesBossLevel_DM300.PylonEnergy.class) > 0){
				return;
			}
	...
	public void dropRocks( Char target ) {
		...
		do {
			safeCell = rockCenter + PathFinder.NEIGHBOURS8[Random.Int(8)];
				...
				|| (Blob.volumeAt(safeCell, CavesBossLevel_DM300.PylonEnergy.class) > 0 && Random.Int(2) == 0));
	...
	public void supercharge(){
		supercharged = true;
		((CavesBossLevel_DM300)Dungeon.level).activatePylon();
		pylonsActivated++;
	...
	protected boolean getCloser(int target) {
		if (super.getCloser(target)){
			...
			if (bestpos != pos){
				...
				Rect gate = CavesBossLevel_DM300.gate;
				for (int i : PathFinder.NEIGHBOURS9){
					if (Dungeon.level.map[pos+i] == Terrain.WALL || Dungeon.level.map[pos+i] == Terrain.WALL_DECO){
						...
						if (!CavesBossLevel_DM300.diggableArea.inside(p)){
