	...
	public static ArrayList<QuickRecipe> getRecipes( int pageIdx ){
		...
		switch (pageIdx){
			...
			case 2:
				...
				result.add(new QuickRecipe(new Digestion_pill.Recipe()));
				result.add(null);
				if (Dungeon.hero != null) {
					...
					if (Dungeon.hero.pointsInTalent(Talent.FLETCH_RECIPE)>=2) {
						result.add(new QuickRecipe(new StuffedMeat.Recipe()));
					}
					if (Dungeon.hero.pointsInTalent(Talent.FLETCH_RECIPE)>=3) {
						result.add(new QuickRecipe(new HoneyArrow.Recipe()));
					}
			...
			case 8:
				...
				result.add(new QuickRecipe(new Remove_Missiles_Curse.Recipe()));
				if (!PixelScene.landscape()) result.add(null);
				result.add(null);
